...
describe('references', () => {
  it('creates a references field with minimal params', () => {
    const result = references({ name: 'related' });
  ...
  it('includes all optional params', () => {
    const result = references({
      name: 'related',
      filter_content_type: [{ name: 'blog' }, { name: 'article' }],
      folder_slug: '/home',
    ...
    expect(result).toEqual({
      ...
      is_reference_type: true,
      source: 'internal_stories',
      filter_content_type: ['blog', 'article'],
      folder_slug: '/home',
      entry_appearance: 'card',
      allow_advanced_search: true,
    });
  ...
  it('transforms filter_content_type component refs to name strings', () => {
    const result = references({
      name: 'items',
      filter_content_type: [{ name: 'page' }],
    });
    expect(result.filter_content_type).toEqual(['page']);
  });

  it('does not include filter_content_type when not provided', () => {
    const result = references({ name: 'items' });
