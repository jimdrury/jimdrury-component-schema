...
function createMockApi(
  ...
  return {
    ...
    createInternalTag: vi.fn().mockResolvedValue({
      data: {
        internal_tag: {
          id: 20,
          name: 'tag',
...
describe('applyPlan', () => {
  ...
  it('creates a missing folder', async () => {
    ...
    expect(api.createComponentFolder).toHaveBeenCalledWith({
      name: 'content',
    });
    expect(result.succeeded).toHaveLength(1);
  ...
  it('creates nested folders with correct parent_id', async () => {
    ...
    const api = createMockApi({
      createComponentFolder: vi
        .fn()
        .mockImplementation(
          async (params: { name: string; parent_id?: number }) => {
    ...
    expect(api.createComponentFolder).toHaveBeenNthCalledWith(1, {
      name: 'layout',
    });
  ...
  it('resolves folder uuid on component when creating both', async () => {
    ...
    expect(api.createComponent).toHaveBeenCalledWith(
      expect.objectContaining({
        component_group_uuid: 'content-uuid',
      }),
    );
    expect(result.succeeded).toHaveLength(2);
  });
